...
namespace tolson.BoosterStation.Service
{
    public class SystemInfoService : Singleton<SystemInfoService>
    {
        ...
        private readonly string PLC_CONFIG_PATH = Config.GetPlcConfigPath();
        private readonly string SYS_CONFIG_PATH = Config.GetSystemConfigPath();
        ...
        private void LoadSysInfo()
        {
            try
            {
                ...
                newInfo.IpAddress = IniConfigHelper.ReadIniData(ConfigSection.通讯参数.ToString(), CommConfigKey.IpAddress.ToString(), "127.0.0.1", PLC_CONFIG_PATH);
                newInfo.CpuType = (CpuType)Enum.Parse(typeof(CpuType), IniConfigHelper.ReadIniData(ConfigSection.通讯参数.ToString(), CommConfigKey.CpuType.ToString(), "S71500", PLC_CONFIG_PATH));
                newInfo.Slot = short.Parse(IniConfigHelper.ReadIniData(ConfigSection.通讯参数.ToString(), CommConfigKey.Slot.ToString(), "0", PLC_CONFIG_PATH));
                newInfo.Rack = short.Parse(IniConfigHelper.ReadIniData(ConfigSection.通讯参数.ToString(), CommConfigKey.Rack.ToString(), "0", PLC_CONFIG_PATH));
                newInfo.HeartBeatAddress = IniConfigHelper.ReadIniData(ConfigSection.通讯参数.ToString(), CommConfigKey.HeartBeatAddress.ToString(), "", PLC_CONFIG_PATH);
        ...
        public bool SetSysInfo(SystemInfo sysInfo)
        {
            ...
            // 写入通讯参数
            result &= IniConfigHelper.WriteIniData(ConfigSection.通讯参数.ToString(), CommConfigKey.IpAddress.ToString(), sysInfo.IpAddress, PLC_CONFIG_PATH);
            result &= IniConfigHelper.WriteIniData(ConfigSection.通讯参数.ToString(), CommConfigKey.CpuType.ToString(), sysInfo.CpuType.ToString(), PLC_CONFIG_PATH);
            result &= IniConfigHelper.WriteIniData(ConfigSection.通讯参数.ToString(), CommConfigKey.Slot.ToString(), sysInfo.Slot.ToString(), PLC_CONFIG_PATH);
            result &= IniConfigHelper.WriteIniData(ConfigSection.通讯参数.ToString(), CommConfigKey.Rack.ToString(), sysInfo.Rack.ToString(), PLC_CONFIG_PATH);
